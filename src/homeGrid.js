...
function renderRooms(nextState) {
  ...
  if (!nextState.rooms.length) {
    roomGrid.append(createElement("p", "empty-note", "No rooms are configured yet."));
    return;
  }
    ...
    gridAlerts.append(
      createElement(
        ...
        hasSiteData(nextState)
          ? "No active incidents are waiting in the queue."
          : "No incidents are recorded because no site data is configured yet."
      )
    );
    return;
  }
...
function renderDevices(nextState) {
  ...
  if (!nextState.devices.length) {
    deviceInventory.append(createElement("p", "empty-note", "No devices are registered yet."));
    return;
  }
...
function renderPlaybooks(nextState) {
  ...
  if (!nextState.playbooks.length) {
    playbookList.append(createElement("p", "empty-note", "No automation playbooks are configured yet."));
    return;
  }
