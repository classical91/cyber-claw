...
function renderAlerts(nextState) {
  ...
  if (!alerts.length) {
    alertFeed.append(createElement("p", "empty-note", "No open alerts. The house is running clean."));
    return;
  }
...
function renderRecommendation(nextState) {
  ...
  if (!entry) {
    latestRecommendation.append(
      createElement("p", "empty-note", "Run a scan in the console to generate the first recommendation.")
    );
    return;
  }
